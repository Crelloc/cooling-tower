...
static int log_info(Log_Pck_Struct *pck) 
{
     ...
     * if sparkfun zigbee shield used, then the following 
     * will send through the xbee.
     */
    XBee.print("Time = ");
    XBee.print(year, DEC);
    ...
    sprintf_f(pck->tempC, buf);
    XBee.print(buf);
    XBee.print(", v_updraft = ");
    sprintf_f(pck->updraftVel, buf);
    XBee.print(buf);
    XBee.print(", inlineFlow = ");
    sprintf_f(pck->inlineFlow, buf);
    XBee.print(buf);
    XBee.print(", v_nozzle = ");
    sprintf_f(pck->nozzleVel, buf);
...
void loop()
{
    ...
    while(!g_update_flag && XBee.available() == 0 ); // it's not time to update sensors and if no xbee message waiting, do nothing
