 ...
 * * Last Modified: 05-3-19 Chris Wallis
 * 
...
#define BUF_SIZE                        6
#define ADC2_ADDRESS                    0x48 // TSI flowmeter and updraft anemometer
...
//#define ELECTRONICS_RH_ADDRESS          0x40 disabled this sensor 5/3/19 to be replaced with BME280
#define BOSCH_RH_ADDRESS                0x77
#define DAC_ADDRESS                     0x62
#define AMB_RH_ADDRESS                  0x45
#define AMB_TEMP_ADDRESS                0x40
...
// XBee's DOUT (TX) is connected to pin 10 (Arduino Mega's Software RX)
// XBee's DIN (RX) is connected to pin 11 (Arduino Mega's Software TX)
...
static double      g_tempC_amb;                         //ambient temperature reading
static double      g_RH_amb;                            //ambient relative humidity
...
static void send_log(Log_Pck_Struct *pck, uint8_t sel){// sel = 0 for XBee; sel = 1 for SD
    ...
    sprintf_f(pck->amb_rh, amb_hum);
    sprintf_f(pck->amb_temp, amb_temp);
    sprintf(buf,"Time = %04d/%02d/%02d %02d:%02d:%02d, "
                ...
                encl_hum, encl_temp, amb_temp, amb_hum
            );
    if(sel == 0) {
        XBee.print(buf);
...
//    g_RH_enclosure = htu.readHumidity(); //no ring buffer necessary
     ...
     * adc value * multiplier for gain 2/3 of ads1115 * m/s coefficient
     * Output is in m/s 
     */
    ...
    log_pck.amb_temp           = g_tempC_amb;
    log_pck.amb_rh              = g_RH_amb;
